        ...
        if ((curr_line >= start_line && curr_line <= end_line) || !hasLine) {
            ...
            while ((pos = strstr(tmp, search_text)) != NULL) {
                ...
                if (beginWith || endWith) {
                    ...
                    /* Write the replace_text into the file and move pointer based on the length of what needs to be replaced. */
                    if (isValid) {
                        char *curr_char = pos;
                ...
                /* No wildcard, normal search and replace by writing until the beginning of search_text,
                write the replace_text into the file, and move the pointer based on the length of search_text. */
                } else {
                    fwrite(tmp, 1, n, outfile);
                    fwrite(replace_text, 1, strlen(replace_text), outfile);
                }
